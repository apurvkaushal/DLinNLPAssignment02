...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_parse = list(partial_parses)
    while len(unfinished_parse) > 0:
        minib=unfinished_parse[0:batch_size]
        transitions=model.predict(minib)
        for j in range(len(transitions)):
            minib[j].parse_step(transitions[j])
        for parse in unfinished_parse:
            if (len(parse.stack) == 1 and len(parse.buffer) == 0):
                unfinished_parse.remove(parse)
