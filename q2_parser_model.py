...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.reglr_placeholder=tf.placeholder(tf.float32)
        
        ### END YOUR CODE

    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=0,reglr=0):
        ...
        """
        ### YOUR CODE HERE
        feed_dict={self.input_placeholder:inputs_batch,self.dropout_placeholder:dropout,self.reglr_placeholder:reglr}
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        wordembed=tf.Variable(self.pretrained_embeddings)
        embeddings=tf.nn.embedding_lookup(wordembed,self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        W=tf.Variable(initial([self.config.n_features*self.config.embed_size,self.config.hidden_size]))
        W2=tf.Variable(initial([self.config.hidden_size,self.config.hidden_size2]))
        U=tf.Variable(initial([self.config.hidden_size2,self.config.n_classes]))
        ...
        h2=tf.nn.relu(tf.matmul(h_drop,W2)+B2)
        h_drop2=tf.nn.dropout(h2,1-self.dropout_placeholder)
        pred=tf.matmul(h_drop2,U)+b2
        
        penalty=tf.reduce_sum(tf.abs(W))
        penalty=penalty+tf.reduce_sum(tf.abs(U))
        return pred,penalty
        ### END YOUR CODE
    

    def add_loss_op(self, pred,penalty):
        ...
        loss=loss+self.reglr_placeholder*penalty
        ### END YOUR CODE
        return loss
